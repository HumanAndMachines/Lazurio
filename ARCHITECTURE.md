...
| **Remote Environment (Vzdálené prostředí)** | Uživatelský název hostované Mašiny: osobní VM nebo pracovní VM Organizace, dosažitelná přes síť Organizace na `launchpad.<mašina>.<org>.lazurio.io` a sesterských adresách. Stejný pojem používají Codex a T3 Code; „VM“ zůstává jen přívlastek, interní pojem je Mašina (decision 0160). |
| **Local Environment (Místní prostředí)** | Uživatelský název tohoto počítače: Mašina, jejíž Launchpad běží jen na loopbacku. Osobní laptop nikdy není Remote Environment (decisions 0155, 0160). |
...
`repository_ids`; vazba workspace na neměnný `github_team_id` a živé ověření
GitHub Team → repository grantu při každém vydání tokenu (fail closed) je
