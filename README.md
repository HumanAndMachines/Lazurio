...
Toto je dnešní podporovaný **Source Root**: důvěryhodný Git checkout Lazuria
přímo v home je současně pracovní Root. Některé existující instalace ještě
nesou historický název složky, například `<home>/Conglomerate`; do řízené
migrace jej není potřeba ručně přejmenovávat. Příkazy se spouštějí přes Bun;
primární checkout lze také zpřístupnit jako uživatelský příkaz `lazurio` v
