...
plus čistou cestu — query a fragment se vždy zahodí; surová cesta se
dekóduje právě jednou a smí obsahovat jen `/` a znaky `A-Z a-z 0-9 . _ ~ -`,
žádný segment `.`/`..`, žádný IPv4/`localhost` literál a nejvýš 512 znaků,
jinak je celé pole `null` (stejně deklarovaná health path); `host` je vždy `null`; v ostatním textu se nahradí každá URL
libovolného schématu i každý IPv4/IPv6 nebo `host:port` literál (včetně
jednoslovného jako `redis:6379`), i když je percent-encoded; `message`
