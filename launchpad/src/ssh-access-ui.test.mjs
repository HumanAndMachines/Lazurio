...
test("SSH access sits next to Chat and opens an accessible three-step dialog", async () => {
  ...
  expect(component).toContain('requestJson("/api/ssh-access")');
  expect(component).toContain("button.hidden = !data?.available");
  expect(component).toContain('"/api/ssh-access/keys"');
  expect(component).toContain('"/api/ssh-access/keys/remove"');
