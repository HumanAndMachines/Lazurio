...
Jeden GitHub User/Operátor má jednu vlastní hostovanou osobní Mašinu. Osobní URL má tvar `<app>.<github-user-login>.lazurio.io`, bez mezivrstvy personal/mašina. Více hostovaných Mašin vyžaduje Organizaci; organizační URL zůstává `<app>.<machine>.<github-org-login>.lazurio.io`. Owner model rozlišuje `user` / `organization`, stabilní GitHub ID a login. DNS slug se při založení odvodí z loginu a normalizuje na lowercase; změna loginu automaticky nepřepojuje runtime DNS. Stávající organizační kontrakt drží frozen runtime_dns_slug a rezervované vyřazené slugy; osobní implementace se musí kvalifikovat a rename/migrace řešit výslovně. Více přístupových laptopů je možné; záloha či replacement není druhé nezávislé osobní prostředí. Buddy označuje roli runtime na osobní VM, nikoli požadavek na další oddělený host nebo výjimku ze singletonu. Osobní Buddy/Personalspace zůstává osobní; org repozitáře a multi-machine práce patří do Organizací, přístup přes SSH. DNS není oprávnění ani veřejný přístup; osobní WireGuard, autentizace a GitHub autorita zůstávají. T3 Code není tímto prohlášen za nasazený a neblokuje první milník WireGuard + SSH/Codex. DNS hostname je case-insensitive: kanonický runtime DNS, konfigurace, certifikát a routing používají lowercase. Proxy, aplikace a owner lookup nesmí rozlišovat pouze case ani vytvářet druhou identitu. GitHub display login smí zachovat case; access identitou je stabilní GitHub ID. Nezakládat dva DNS záznamy či alias identity jen kvůli casing. Acceptance musí prokázat, že mixed-case a lowercase hostname vedou na stejnou službu se stejnými auth/WG pravidly; runtime ověření zatím není doložené.
