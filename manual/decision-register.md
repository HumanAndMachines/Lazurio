...
| 0129 | Jediný obecný update je explicitní `lazurio update`: sekvenčně Lazurio Root → Organization Rooty → čerstvě rediscoverovaná namountovaná org-level repa a Workspace Moduly, vždy clean `main` a ff-only. Dirty tracked/untracked práce jde do ověřeného neobnovovaného recovery stashe; wrong branch se vrací na `main` se zachováním commitů. Ahead/diverged main, nebezpečný detached stav a probíhající Git operace jsou blocked s Codex promptem. Jeden nevalidní slot je karanténa, ne globální `inventory_unavailable`: zdraví sourozenci pokračují a rename/transfer uvnitř jedné Lazurio Organization řeší Agent přes guarded check + exact-fingerprint apply, který zachová Git data a nevytvoří duplicitní clone. Cross-Organization přesun je explicitní source/access migrace a bez budoucí neměnné migrační identity se z Organization-scoped slugu ani remote redirectu nehádá. Productionspace, Personalspace, worktrees a root-space repository-db jsou vyloučené. CLI, Launchpad Sync a legacy adaptéry volají tentýž engine; první render je GET-only bez fetch. |
