...
| 0136 | Produkční localhost Lazurio Root má jedinou filesystem autoritu odvozenou z OS home: `<home>/Lazurio` (`~/Lazurio` na macOS/Linuxu, `%USERPROFILE%\Lazurio` na Windows). Top-level installer ani package-managed CLI nevytváří picker, persisted root selection, config override ani druhý locator; absolutní cesta záměrně nese uživatele Mašiny. Package-only Root je platný bez source checkoutu. Development profil používá právě jeden ověřený checkout `<home>/Lazurio/development/Lazurio` a explicitní source link stejného CLI/Core; generátor nevytváří druhou runtime kopii. Legacy Root v jiné home cestě není alternativní aktivní Root: samostatná detekovaná migrace jej musí bezpečně převést do canonical targetu bez tiché adopce nebo ručního přesunu. |
