...
| 0131 | Každý worktree založený Task Agentem zachytí do gitignored lokálního sidecaru samodeklarované recovery vodítko `Mašina + harness surface + opaque task/thread/session/chat ID` (`conversation_origin.machine_ref` + `surface` + `thread_id`); agentní create lane bez dohledatelného harness ID failuje zavřeně. Vodítko slouží Principálovi ke spojení lokálního výstupu s chatem a k obnově rozdělané práce. Je editovatelné, podvrhnutelné a může zastarat: není ověřenou identitou, podpisem, atribucí, oprávněním, auditním důkazem ani organizační telemetrií. GitHub zůstává jedinou autoritou přístupů a commitové atribuce. Automatizace bez Task Agenta používají `not_applicable`; vědomý handoff smí vodítko přepsat údaji nové relace, nedostupný thread ale sám nedokazuje opuštění práce. |
