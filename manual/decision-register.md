...
| 0133 | Dependency autoritu Workspace App tvoří její manifestem deklarovaný package root a verzovaný lockfile, nikdy filesystem mtime ani lokální `.env`. Relativní `file:` smí read-only odkazovat na přesný kanonický package cíl uvnitř stejné Organizace; mutation/ancestor hranicí zůstává owning checkout a jiný symlink cíl, cross-Organization přesah nebo změna cílové package identity failuje zavřeně. Změna repozitáře, který lokální cíl vlastní, zahrne do post-update obnovy i jeho jinak nezměněné App consumery. `Install` zachová existující strom; `Repair` provede transakční čistou frozen instalaci přesného `node_modules` s rollbackem. Po skutečné změně source tentýž mechanismus automaticky ověří dotčené Apps a při prvním selhání jednou zkusí čistou opravu. Životní cyklus běžící managed aplikace vlastní Launchpad; neplatný lockfile zůstává source chybou pro Agenta. |
