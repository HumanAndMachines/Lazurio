...
function usage() {
  return [
    ...
    "  lazurio migrate organization-manifest <organization-root> [--write] [--finalize] [--json]",
    "    bez --write: jen plán (stav před/po, hashe, změny); --write: legacy → transition v čistém task worktree",
    "    --finalize: transition → current; zápis zůstává blokovaný reader/update gatem (decision 0145)",
